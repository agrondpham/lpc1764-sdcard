...
/************************************************************************																										
** 函数功能: 等待SD卡回应	
** 入口参数: Response:要得到的回应值	    		
** 出口参数: 返回值:0,成功得到了该回应值,其他回应值失败		
** 备    注: 无
************************************************************************/
...
/************************************************************************																										
** 函数功能: 向SD卡发送一个命令	 结束是不失能片选，还有后续数据传来
** 入口参数: u8 cmd   命令 
**			 u32 arg  命令参数
**	    	 u8 crc   crc校验值	
** 出口参数: SD卡返回的响应			
...
** 入口参数: u8 *cid_data(存放CID的内存，至少16Byte）
** 出口参数: 0：NO_ERR
**			 1：TIME_OUT
**			 other：错误信息
** 备    注: 无
...
/************************************************************************																										
** 函数功能: 读SD卡的一个block
** 入口参数: u32 sector 取地址（sector值，非物理地址）
...
u8 MSD_WriteBuffer(u8* pBuffer, u32 WriteAddr, u32 NumByteToWrite)
{
	...
	while (NbrOfBlock--)//写入一个扇区
	{
	    ...
	    ssp0_send(0xFE);//放起始令牌0xFE   
	    //放一个sector的数据
	    for(i=0;i<512;i++)ssp0_send(*pBuffer++);  
	    //发2个Byte的dummy CRC
	    ssp0_send(0xff);
	    ssp0_send(0xff); 
...
//////////////////////////////////////////////////////////////////////////
/************************************************************************																										
** 函数功能: 写入SD卡的一个block(未实际测试过) 
** 入口参数: u32 sector 扇区地址（sector值，非物理地址）
**			 u8 *buffer 数据存储地址（大小至少512byte）
** 出口参数: 0： 成功
**			 other：失败
** 备    注: 无
...
/************************************************************************																										
** 函数功能: 读SD卡的多个block(实际测试过) 
** 入口参数: 输入:u32 sector 扇区地址（sector值，非物理地址）
**			 u8 *buffer 数据存储地址（大小至少512byte）
...
/************************************************************************																										
** 函数功能: 在指定扇区,从offset开始读出bytes个字节 
** 入口参数: u32 sector 扇区地址（sector值，非物理地址）
...
u8 SD_Read_Bytes(unsigned long address,unsigned char *buf,unsigned int offset,unsigned int bytes)
{
	...
	for(i=0;i<offset;i++)ssp0_send(0xff);//跳过offset位 
    for(;i<offset+bytes;i++)*buf++=ssp0_send(0xff);//读取有用数据	
    for(;i<512;i++) ssp0_send(0xff); 	 //读出剩余字节
    ssp0_send(0xff);//发送伪CRC码
    ssp0_send(0xff);  
    SD_CS(1);//关闭SD卡
	return 0;
}
