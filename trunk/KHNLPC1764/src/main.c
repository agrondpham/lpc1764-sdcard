	...
	} else if (IIRValue == IIR_RDA) /* Receive Data Available */
	{
		...
		switch (data)
		{
			case 0x0A:
			{
				...
				{
				 if (rx_buffer0[0]==NULL)  flag_system.start_rx_gsm = 1;
					else if (rx_buffer0[0] == '+' && rx_buffer0[1] == 'C' && rx_buffer0[2] == 'M' && rx_buffer0[3] == 'G' && rx_buffer0[4] == 'R')
					{
       ...
       break;
		...
		case 0x0D:        break;     // enter thi bo qua
		default:
		{
...
void UART1_IRQHandler(void) {
	...
	if (flag_system.request_data)
	{
		...
		if (rx_buffer1[0] == 'G' && rx_buffer1[1] == 'P' && rx_buffer1[2] == 'R' && rx_buffer1[3] == 'M' &&rx_buffer1[4] == 'C')
		{
     ...
     unsigned char i,j;
        if (apn[0]==NULL || apn[0]==0xff)
        {
				...
				if (ipServer[0]==NULL || ipServer[0]==0xff)
         {
				 ...
				 if (timeCheck[0]==NULL || timeCheck[0]==0xff) counter_send_gps = 20;// gui len server
	...
	while(1)
	{
		if (flag_modem.modem == not_connect)
		{
			 ...
			 if (flag_gprs.status_gprs != ok_status)
         {
            ...
            flag_gprs.status_gprs = init_tcp (apn, userName, passApn, flag_gprs.status_gprs);
            resend++;
            ...
            else if (flag_gprs.status_gprs == ok_status) counter_init=0;
		...
		if (flag_modem.modem == connected)
		{
            ...
            else if (ipServer[0]!=NULL && ipServer[0] != 0xFF&& tcpPort[0] != NULL) // thieu xac nhan cac thong so
            {
              ...
              {                                           // dung tam bien vitri de lam bien dem
                if (imei[vitri] < 0x30 || imei[vitri] > 0x39)
                {
                  ...
                  else vitri=1;    // gan = 1 de stop gui
                  break;
                }
              }
               ...
               if (connectServer (ipServer, tcpPort) == ok_status)
                {
                   flag_gprs.connect_ok = 1;   // ket noi ok
                    ...
                    memset(data_gps,gpsLen,NULL);    // xoa NULL het GPS data
										...
										sprintf(data_gps, "$$KHN$%s,56,%s,%s,N,%s,E,%s,%d,%d,%d,%d,%s,%s,%d,%d,%s$",version ,imei, latitude, longitude, speed_gps, oil_value, flag_system.status_door, flag_system.status_key, flag_system.card_status, so_vin, phoneDrive,flag_system.cold_hot, flag_system.sleep,time_gps_send);// trang thai IN3
